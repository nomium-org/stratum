...
pub fn log_share_hash(location: &str, share: &ShareLog) {
    ...
    info!("Используем базовую директорию: {:?}", base_dir);

    let log_dir = base_dir.join("debug_logs");
    info!("Пытаемся создать директорию логов: {:?}", log_dir);

    // Создаём директорию с логированием результата
    ...
    info!("Путь к файлу лога: {:?}", file_path);
    ...
    // Проверяем существование файлового дескриптора
    if !handles.iter().any(|(loc, _)| loc == location) {
        info!("Создаём новый файловый дескриптор для {}", location);
        
        // Открываем новый файл с добавлением
        ...
        {
            Ok(file) => {
                info!("Файл успешно открыт: {:?}", file_path);
